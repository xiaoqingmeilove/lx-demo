    ...
    <div class="model_box">
        <el-form ref="roleForm" :rules="rules" :model="form" label-width="150px">
            ...
            <el-form-item label="区域编码" prop="code">
                <el-input v-model="form.code" placeholder="请输入区域编码" disabled/>
            </el-form-item>
            ...
            <el-form-item label="运输里程（公里）" prop="kilometers">
                <el-input-number style="width:100%" v-model="form.kilometers" controls-position="right" :min="1" />
            </el-form-item>
            <el-form-item label="运费单价（元/吨*公里）" prop="fullLoadPrice">
                <el-input-number style="width:100%" v-model="form.fullLoadPrice" controls-position="right" :min="1" />
            </el-form-item>
            <el-form-item label="运费零但单价（元/吨*公里）" prop="lessThanPrice">
                <el-input-number style="width:100%" v-model="form.lessThanPrice" controls-position="right" :min="1" />
            </el-form-item>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                parentId:[{ required:false, message:'请选择所属区域'}],
            }
        }
    },
    methods:{
        ...
        save() {
            this.$refs.roleForm.validate(valid => {
                if (valid) {
                    console.log(this.form,this.parentId)
                    if(this.parentId){
                        this.form.parentId = this.parentId
                        apiSystem.editRegionDatails(this.form,this.parentId).then(res=>{
    ...
    created() {
        if (this.roleItem?.code) {
            this.parentId = Number(this.roleItem.parentId)
            const { id, name, sunId, belongName, code, remark, sort,startPoint,endPoint,routeName,kilometers,fullLoadPrice,lessThanPrice,flag } = this.roleItem
            this.form = { id, name, sunId, belongName, code, remark, sort,startPoint,endPoint,routeName,kilometers,fullLoadPrice,lessThanPrice,flag }
        }
    }
}
</script>
