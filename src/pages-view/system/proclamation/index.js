...
export default {
  ...
  methods: {
    showDetail(row){
     ...
     this.noticeForm = row;
     this.editNoticeState = row.noticeState === 0 ?  true :false;
    },
    deleteNotice(row) {
      VXETable.modal
        .confirm({
    ...
    exportExec() {
      ...
      apiSystem
        .getProclamationListExport(this.condition)
        .then((res) => {
          if (res) {
            blobToFile(
              res,
              `${this.$route.meta.title}_${moment().format(
                "YYYYMMDDHHmm"
              )}.xlsx`
            );
          } else {
