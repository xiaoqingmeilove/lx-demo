...
export default {
  ...
  activated() {
    this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '竞价邀请';
    this.getList();
  },
  ...
  methods: {
    ...
    exprot(){
      ...
      apiBid.getSupplierExport({...this.condition}).then(res => {
        blobToFile(res, `导出${findMenuByCode(this.menuCode)?.title??'竞价邀请'}_${moment().format("YYYYMMDDHHmm")}.xlsx`);
      }).finally(() => {
        setTimeout(() => {
          loading.close();
        }, 500)
    ...
    getList(callback) {
      ...
      apiBid.getJournalingBidDetails(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
    ...
    // 一览表双击事件 查看详情
    onCellDbClick({row}) {
      this.$router.push({ path: `/bidding/bidInvite-detail/detail/${row.id}` });
    },
    // 查询
    onSearch(value) {
