...
export default {
  ...
  methods: {
    ...
    getList(callback) {
      ...
      apiBid.getHallList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
    ...
    // 一览表双击事件 查看详情
    onCellDbClick({row}) {
      this.$router.push({ path: `/bidding/supplierApply-detail/detail/${row.id}` });
    },
    // 查询
    onSearch(value) {
