...
export default {
    ...
    activated() {
        this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '我的投标';
        this.getList();
    },
    ...
    methods: {
        ...
        getList(callback) {
            ...
            apiBiddingManagement.getBidList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
        ...
        // 一览表双击事件 查看详情
        onCellDbClick({ row }) {
            this.$router.push({ path: `/biddingManagement_myTender_detail/${row.id}` });
        },
        // 查询
        onSearch(value) {
