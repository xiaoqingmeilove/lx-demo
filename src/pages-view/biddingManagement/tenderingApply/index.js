...
export default {
    ...
    activated() {
        this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '招标项目申请';
        this.getList();
    },
    ...
    methods: {
        ...
        getList(callback) {
            ...
            apiBiddingManagement.getTenderList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
        ...
        onCellDbClick({ row }) {
            if (row.billState > 1) {
                this.$router.push({ path: `/biddingManagement/tenderingApply-detail/${row.id}` });
            } else {
                this.$router.push({ path: `/biddingManagement/tenderingApply-edit/${row.id}` });
            }
        },
        // 查询
        onSearch(value) {
