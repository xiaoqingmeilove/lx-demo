  ...
  <app-page>
    <bill-btns :form="form" title="招标申请">
      <template #buttons>
        ...
        <template v-else>
          <!-- <page-button @click="onSubmitBtnClick" v-if="showBtn('submitWorkflow')">{{ showBtn("submitWorkflow")?.btnName ?? "提交审批" }}</page-button> -->
          ...
          <page-button @click="submitBidding" v-if="showBtn('submitBidding')">{{ showBtn("submitBidding")?.btnName ?? "确定发标" }}</page-button>
        ...
        <page-button @click="$tabs.close()" v-if="showBtn('close')">{{ showBtn("close")?.btnName ?? "关闭"   +  form.billState}}</page-button>
        <workflow-operation
          code="market_srm_tender_audit"
          :bill-id="form.id"
          :bill-no="form.billNo"
          :bill-state="form.billState"
          :show-btns="showWorkBtns('workflowBtn')"
          :read-only="false"
          @ok="onWorkflowOk"
          ref="workflow"
        ></workflow-operation>
