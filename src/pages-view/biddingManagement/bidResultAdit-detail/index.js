...
export default {
    ...
    computed: {
        ...
        showWorkBtns() {
            return (parentId = "workflowBtn") => {
              let parentFind = this.allBtnControlList.find(
                (x) => x.btnCode == parentId
              );
              if (parentFind?.children.length) {
                return (parentFind.children ?? []).map((x) => x.btnCode);
              }
              return [];
            };
          },
    ...
    data() {
        ...
        return {
            ...
            noticeForm: {},
            activeName: 'db', // 默认选中
            activeNameList: ['jbxx', 'cgmx', 'gys','zbwj','dyzx','pbDetail','swtp','db'],
    ...
    methods: {
        ...
        getDetail() {
            ...
            let id = this.id;
            apiBiddingManagement.getTenderAuditDetail({ id }).then((res) => {
                if (res.code === 200 && res.data) {
                    this.form = { ...res.data };
        ...
        showBtnConfig() {
            ...
            apiQuotation.postDetailBtnList('bidResultAdit', data).then((res) => {
                if (res.code === 200) {
