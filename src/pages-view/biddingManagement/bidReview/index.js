...
export default {
    ...
    data() {
        ...
        return {
            ...
            sourceList: {
                ...
                scoreStatusList: (dict["score_status"] || []).map((d) => {
                    return { label: d.dictLabel, value: d.dictValue };
                }),
            },
            visible: {
    ...
    activated() {
        this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '标书评审';
        this.getList();
    },
    ...
    methods: {
        ...
        getList(callback) {
            ...
            apiBiddingManagement.getBidReviewList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
        ...
        // 一览表双击事件 查看详情
        onCellDbClick({ row }) {
            this.$router.push({ path: `/biddingManagement/bidReview-detail/${row.id}` });
        },
        // 查询
        onSearch(value) {
