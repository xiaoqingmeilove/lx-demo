    ...
    methods: {
        init() {
            ...
            this.createForm = {
                ...
                originalPaymentTerm:this.supplierList.find(item => item.checked).paymentTerm,
                fileList:[]
            }
        },
                ...
                if( res.code === 200 ) {
                    ...
                    if(this.supplierList.length > 0) {
                       ...
                       if(!supplier) {
                        ...
                        this.createForm.originalPaymentTerm = this.supplierList[0].paymentTerm;
                       }
                    }
        ...
        validateForm() {
            const requiredFields = ['negotiateDate', 'negotiateTime', 'place', 'participants', 'content', 'amount','paymentTerm','fileList'];
            let count = 0
            for (let field of requiredFields) {
                const value = this.createForm[field];
            ...
            if(count && count == requiredFields.length) {
                this.$message.error('请填写谈判信息');
                return false;
            } 
        ...
        handleNegotiateRecordClick(item) {
            ...
            this.createForm.originalPaymentTerm = this.supplierList.find(item => item.checked).originalPaymentTerm;
