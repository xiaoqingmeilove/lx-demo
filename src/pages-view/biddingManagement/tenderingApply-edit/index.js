...
export default {
    ...
    mixins: [mixin_table],
    components: { VueEasyLightbox, columnPasting, productList, materialList, addUnconventional, timeline, supplierList, expertList },
    computed: {
        ...mapState({
        ...
        bindDescItems() {
            if (!this.id) {
                return this.descItems.filter(x => x.field !== 'bizTime')
            }
    ...
    data() {
        ...
        return {
            ...
            rulesobj: {
                ...
                bidScope: [{ required: true, message: "请选择招标范围" }],
                purchaseUserName: [{ required: true, message: "请选择招标范围" }],
                purchaseUserTel: [{ required: true, message: "请选择招标范围" }],
                purchaseType: [{ required: true, message: "请选择招标范围" }],
                bidScope: [{ required: true, message: "请选择招标范围" }],
            },
            ...
            descSourceList: {
                ...
                tenderEvaluationMethod: (dict["tender_evaluation_method"] ?? []).map((d) => {
                    return { label: d.dictLabel, value: d.dictValue };
                }),
            },
            defaultColumns: [],
    ...
    methods: {
        checkForm(){
            this.form = {
                ...this.form,
                "purchaseType": (this.dict["tender_purchase_type"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                "purchaseTypeName": (this.dict["tender_purchase_type"] ?? []).find(d => d.defaultFlag == 0)?.dictLabel,
                "purchaseMethod": (this.dict["tender_purchase_method"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                "purchaseMethodName": (this.dict["tender_purchase_method"] ?? []).find(d => d.defaultFlag == 0)?.dictLabel,
                "bidScope": (this.dict["tender_bid_scope"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                "bidScopeName": (this.dict["tender_bid_scope"] ?? []).find(d => d.defaultFlag == 0)?.dictLabel,
                "pricingMethod": (this.dict["tender_pricing_method"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                "pricingMethodName": (this.dict["tender_pricing_method"] ?? []).find(d => d.defaultFlag == 0)?.dictLabel,
                "projectStatus": (this.dict["tender_project_status"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                "projectStatusName": (this.dict["tender_project_status"] ?? []).find(d => d.defaultFlag == 0)?.dictLabel,
                "entrustType": (this.dict["tender_entrust_type"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                "entrustTypeName": (this.dict["tender_entrust_type"] ?? []).find(d => d.defaultFlag == 0)?.dictLabel,
                ruleMap: {
                    tenderOpeningType: (this.dict["tender_opening_type"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                    tenderEvaluationType: (this.dict["tender_evaluation_type"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                    tenderEvaluationMethod: (this.dict["tender_evaluation_method"] ?? []).find(d => d.defaultFlag == 0)?.dictValue,
                    "technologyRatio":50,
                    "businessRatio":50,
                    "bidResultAuditFlag":1
                },
            }
        },
        ...
        waste(){
            VXETable.modal.confirm({
                ...
                apiBiddingManagement.putTenderCancel(params).then((res) => {
                    if (res.code === 200 && res.data) {
                        this.$tabs.close(null, `/biddingManagement/tenderingApply-detail/${this.id}`)
                    } else {
        ...
        setZbjhsj(){
            if(this.id && this.form.ruleMap?.length){
                return
            }
            this.form.ruleMap = {
                ...this.form.ruleMap,
                ...Object.fromEntries(this.zbjhsjColumns.map((item, index) => [item.field, moment().add(index, 'days').format("YYYY-MM-DD")]))
            }
        ...
        getSelectList() {
            apiSystem.getOrderUsersList().then(res => {
                ...
                // 遍历数据，如果userId匹配就自动赋值
                if (currentUserId) {
                    const matchedUser = descSourceList.inviteUserList.find(user => user.purchaseUserId === currentUserId);
                    if (matchedUser) {
                        // 自动赋值到form中
                        this.$set(this.form, 'purchaseUserName', matchedUser.label);
                        this.$set(this.form, 'purchaseUserId', matchedUser.purchaseUserId);
                        this.$set(this.form, 'purchaseUser', matchedUser.purchaseUser);
                        this.$set(this.form, 'purchaseUserTel', matchedUser.purchaseUserTel);
                    }
                }
                this.descSourceList = { ...descSourceList };
            })
        },
        onZbjhsjChange(e,item) {
            const currentIndex = this.zbjhsjColumns.findIndex(column => column.field === item.field);
            if (currentIndex >= 0) {
                this.zbjhsjColumns.map((item,index)=>{
                    if(index > currentIndex){
                        const currentDate = moment(e).add(index - currentIndex , 'days').format("YYYY-MM-DD");
                        this.form.ruleMap[item.field] = currentDate;
                    }
                })
            }
        },
        ...
        onCanSelectDetail({ row }) {
            return Number(row.dataSource) != 2; // 返回 true 表示允许选择，false 表示禁用
        },
        addUnconOk(data) {
            let row = { ...data };
            let detailList = [...this.form.detailList];
            row.hasOwnProperty('_X_ROW_KEY') && delete row._X_ROW_KEY;
            row.purchaseDate = row.latestArrivalDate
        ...
        addTender() {
            ...
            apiBiddingManagement.addTender(this.form).then((res) => {
                if (res.code === 200 && res.data) {
                    ...
                    this.$tabs.close(
                        null,
                        `/biddingManagement/tenderingApply-edit/${this.id}`
                    );
                } else {
                    this.$message.error(res.message);
                }
        ...
        getDetail() {
            ...
            let id = this.id;
            apiBiddingManagement.getTenderDetail({ id }).then((res) => {
                if (res.code === 200 && res.data) {
                    this.form = { ...res.data };
                    if (res.data.billState > 1) {
                        this.$router.push({ path: `/biddingManagement/tenderingApply-detail/${id}` });
                    }
        ...
        async setColumn() {
            ...
            let data = {
                code: "detailList,detailList2,detailList3",
            ...
            let dataForm = {
                code: "detailForm,detailForm2,detailForm3",
                corpCode: this.businessCode ?? "LX",
            };
        ...
        addExpertData(data) {
            ...
            const list = dataClone.filter(f => !expertListData.some(s => s.employeeId == f.employeeId));
            if (!list.length) {
        ...
        async onSubmitBtnClick() {
            ...
            if(!this.form.supplierList.length){
                this.$message.error('请添加参与供应商');
                return 
            }
            this.beforeSubmit();
        },
        onWorkflowLoad() {
            console.log('onWorkflowLoad');
        },
        onWorkflowClose() {
            console.log('onWorkflowClose');
        },
        onWorkflowOk() {
            //TODO 刷新表单数据
            console.log('ok');
