...
export default {
  ...
  activated() {
    this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '采购订单确认';
    this.getList();
  },
  ...
  methods: {
    confirm() {
      ...
      apiContract.putPurchaseOrderConfirmAgree({ ids }).then(res => {
        if (res.code === 200) {
          this.getList();
    ...
    reject() {
      ...
      const loading = this.loading('请求中...');
      const ids = list.map(d => d.id);
      apiContract.putPurchaseOrderConfirmReject({ ids }).then(res => {
        if (res.code === 200) {
          this.getList();
          this.$message.success(res?.message ?? '操作成功')
        } else {
    ...
    exprot() {
      ...
      apiContract.getSupplierExport({ ...this.condition }).then(res => {
        blobToFile(res, `导出${findMenuByCode(this.menuCode)?.title ?? '采购订单确认'}_${moment().format("YYYYMMDDHHmm")}.xlsx`);
      }).finally(() => {
        setTimeout(() => {
          loading.close();
        }, 500)
    ...
    getList(callback) {
      ...
      apiContract.getPurchaseOrderConfirmList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
    ...
    // 一览表双击事件 查看详情
    onCellDbClick({ row }) {
      this.$router.push({ path: `/purchaseOrder/orderConfim-detail/detail/${row.id}` });
    },
    // 查询
    onSearch(value) {
