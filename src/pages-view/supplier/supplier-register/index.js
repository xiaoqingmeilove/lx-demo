...
export default {
  ...
  activated() {
    this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '供应商注册';
    this.getList();
  },
  ...
  methods: {
    importOk(e){
      ...
      apiBasic.postSupplieImportRregister(e[0]).then(res => {
        if(res.code === 200){
          this.getList();
        }else{
    ...
    add(){
      this.$router.push({ path: "/supplier/supplierRegister-detail/detail" });
    },
    closePopup(){
    ...
    exprot(){
      ...
      apiBasic.getSupplierExport({...this.condition}).then(res => {
        blobToFile(res, `导出${findMenuByCode(this.menuCode)?.title??'供应商注册'}_${moment().format("YYYYMMDDHHmm")}.xlsx`);
      }).finally(() => {
        setTimeout(() => {
          loading.close();
        }, 500)
    ...
    getList(callback) {
      ...
      apiBasic.getSupplierList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
    ...
    // 一览表双击事件 查看详情
    onCellDbClick({row}) {
      this.$router.push({ path: `/supplier/supplierRegister-detail/detail/${row.id}` });
    },
    // 查询
    onSearch(value) {
