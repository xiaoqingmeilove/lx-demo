...
export default {
  ...
  components: { VueEasyLightbox, productList, materialList },
  ...
  data() {
    ...
    return {
      menuCode: "supplier_supplierAccess",
      editState: false,
      backForm: {},
      moment,
      columns: [],
  ...
  mounted() {},
  activated() {
    this.$tabs.activeTab.title = `${findMenuByCode(this.menuCode)?.title ?? '供应商准入'}详情`;
    this.init();
  },
  methods: {
    ...
    async closePage() {
      ...
      if (res === "confirm") {
        ...
        if (this.editState) {
          const res = await apiBasic.putAccessEdit(this.form);
          loading.close();
          if (res.code === 200) {
            this.editState = false;
    ...
    validate(){
      ...
      }else if(this.form.bankList && this.form.bankList.length && this.form.bankList.some(x=>!x.bankName || !x.accountNo)){
        flag = false;
        this.$message.warning('请填写开户银行信息');
      }else if(this.form.contactList && this.form.contactList.length && this.form.contactList.some(x=>!x.bankName || !x.accountNo || !x.accountName || !x.branchCode)){
        flag = false;
        this.$message.warning('请填写联系人信息');
      }
      return new Promise((resolve, reject) => {
    ...
    async editSave(){
      const loading = this.loading('保存中...');
      apiBasic.putAccessEdit(this.form).then(res=>{
        if(res.code === 200){
          this.getDetail();
          this.editState=false;
          this.$message.success(res?.message || '保存成功');
    ...
    getDetail() {
      ...
      apiBasic.getAccessDetail(this.id).then((res) => {
        if (res.code === 200) {
    ...
    async setColumn() {
      const res = await apiQuotation.getColumnsConfigMulti(this.menuCode, {code: "detailList,detailList1,detailList2,detailList3",corpCode: this.businessCode ?? "LX"});
