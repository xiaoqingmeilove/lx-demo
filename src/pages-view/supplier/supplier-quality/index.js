...
export default {
  ...
  data() {
    ...
    return {
      dict,
      menuCode: "supplier_supplierQuality",
      columns: [],
      tableColumns: [],
      defaultColumns: [],
      searchOptions: [],
  ...
  activated() {
    this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '供应商资质';
    this.getList();
  },
  ...
  methods: {
    ...
    exprot(){
      ...
      apiBasic.getQualificationsExport({...this.condition}).then(res => {
        blobToFile(res, `导出${findMenuByCode(this.menuCode)?.title??'供应商资质'}_${moment().format("YYYYMMDDHHmm")}.xlsx`);
      }).finally(() => {
        setTimeout(() => {
          loading.close();
        }, 500)
    ...
    getList(callback) {
      ...
      apiBasic.getQualifications(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
