      ...
      <div class="app-table-container" auto>
        ...
        <div class="app-table-body">
          ...
          >
            ...
            <template #masterBillNo="{row, column}">
              <span style="color: var(--base-color);cursor: pointer;" @click="toSupplierRegister(row)">{{row[column.field]}}</span>
            </template>
            ...
            <template #supplierStatus="{row, column}">
              <span>
                <span class="row-bg" :style="{background: getColorAndName(row, column, sourceList.supplierStateList).color}">{{getColorAndName(row, column, sourceList.supplierStateList).label}}</span>
              </span>
            </template>
