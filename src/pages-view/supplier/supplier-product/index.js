...
export default {
  ...
  data() {
    ...
    return {
      dict,
      menuCode: "supplier_supplierProduct",
      columns: columns,
      tableColumns: [],
      defaultColumns: [],
  ...
  activated() {
    this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '供应商产品管理';
    this.getList();
  },
  ...
  methods: {
    ...
    exprot() {
      ...
      apiBasic.getdetailExport({ ...this.condition, ids }).then(res => {
        blobToFile(res, `导出${findMenuByCode(this.menuCode)?.title ?? '供应商产品管理'}_${moment().format("YYYYMMDDHHmm")}.xlsx`);
      }).finally(() => {
        setTimeout(() => {
          loading.close();
        }, 500)
    ...
    getList(callback) {
      ...
      apiBasic.getdetail(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
