...
export default {
  ...
  computed: {
    ...
    remainDay(){
      return (row) => {
        ...
        return moment(validityEndDate).diff(moment(), 'days')
      }
    },
  },
  data() {
  ...
  methods: {
    ...
    importOk(e){
      ...
      apiBasic.postSourceImport(e[0]).then(res => {
        if(res.code === 200){
          this.getList();
        }else{
    ...
    getList(callback) {
      ...
      apiBasic.getSourceList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
    ...
    // 一览表双击事件 查看详情
    onCellDbClick({row}) {
      this.$router.push({ path: `/material/sourceList-detail/detail/${row.id}` });
    },
    // 查询
    onSearch(value) {
