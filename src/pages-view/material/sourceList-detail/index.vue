<template>
  <app-page>
      ...
      <template #buttons>
        <template v-if="id">
          <template v-if="editState">
            ...
            <page-button v-if="showBtn('editClose')" @click="cancel">取消</page-button>
          </template>
          <template v-else>
            ...
            <page-button style="background:red" v-if="showBtn('cancel')" @click="visible.zfVisible=true">作废</page-button>
          </template>
        </template>
        <template v-else>
        ...
        <workflow-operation
          code="market_material_source_list"
          :bill-id="form.id"
          :bill-no="form.billNo"
        ...
        <el-tab-pane name="jbxx">
          ...
          >
            <template #solt_date="{item}">
              ...
              <template v-else>
                <span>{{formatDate(form[item.field])}}</span>
              </template>
            </template>
          </data-form>
          <sub-title title="物料信息">
            ...
            <template #buttons>
              <vxe-toolbar ref="cpxxtoolbar" class="app-table-toolbar">
                <template #buttons>
                  <page-button v-if="editState" @click="visible.addProductList = true" content="选择物料" type="text" style="margin-right: 36px;">
          ...
          <vxe-grid
            ...
            :data="bindTableData"
            :id="`tb_material_sourceList_cpxx_list_${userInfo.userId}`"
          ...
          >
            ...
            <!-- <template #action="{row, rowIndex}">
              <page-button type="text" @click="delRow(row, rowIndex, 'detailList')">删除</page-button>
            </template> -->
          </vxe-grid>
          <sub-title title="配额列表">
            <template #beforeIcon>
              <i
                :class="
                  formDataShow.pelb ? 'el-icon-caret-bottom' : 'el-icon-caret-right'
                "
                @click="formDataShow.pelb = !formDataShow.pelb"
              ></i>
            </template>
            <template #buttons>
              <vxe-toolbar ref="pelbtoolbar" class="app-table-toolbar">
                <template #buttons>
                  <page-button v-if="editState" @click="addSupplier" content="选择供应商" type="text" style="margin-right: 36px;">
                    <template #img><svg-icon icon-class="addIcon" style="margin-right: 5px" /></template>
                  </page-button>
                </template>
                <template #tools>
          ...
          <vxe-grid
            v-show="formDataShow.pelb"
            :data="form.supplierList"
            :id="`tb_material_sourceList_pelb_list_${userInfo.userId}`"
            :custom-config="{ storage: true }"
          ...
          >
            ...
            >
              <template v-if="editState && form.type">
                <vue2-number-input
            ...
            <template #action="{row, rowIndex}">
              <page-button type="text" @click="delRow(row, rowIndex, 'supplierList')">删除</page-button>
            </template>
          </vxe-grid>
        </el-tab-pane>
    ...
    <template #custom>
      <BecomeInvalidRemind :visible="visible.zfVisible" :list="{ billNo: form.billNo }" @ok="becomeInvalid" @cancel="visible.zfVisible=false"/>
      <vxe-modal
        width="80%"
