...
export default {
  ...
  activated() {
    this.$tabs.activeTab.title = findMenuByCode(this.menuCode)?.title ?? '合同列表';
    this.getList();
  },
  ...
  methods: {
    add(){
      this.$router.push({ path: "/contract/contractApply-detail/detail" });
    },
    closePopup(){
    ...
    exprot(){
      ...
      apiContract.getSupplierExport({...this.condition}).then(res => {
        blobToFile(res, `导出${findMenuByCode(this.menuCode)?.title??'合同列表'}_${moment().format("YYYYMMDDHHmm")}.xlsx`);
      }).finally(() => {
        setTimeout(() => {
          loading.close();
        }, 500)
    ...
    getList(callback) {
      ...
      apiContract.getContractBillList(this.condition, this.tablePage.currentPage, this.tablePage.pageSize).then(res => {
    ...
    // 一览表双击事件 查看详情
    onCellDbClick({row}) {
      this.$router.push({ path: `/contract/contractApply-detail/detail/${row.id}` });
    },
    // 查询
    onSearch(value) {
