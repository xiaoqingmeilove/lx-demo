<template>
  <div class="filelist-group">
    <div class="filelist-group-aside">
      ...
      <div v-if="!readOnly && editable" class="filelist-group-list-item filelist-group-action"
        @click="onAddFolderClick"
      >
        <svg-icon icon-class="folder-add" fill="#a4a4a4" size="22" class="filelist-group-icon"></svg-icon>
        新建目录
      </div>
    </div>
    <div class="filelist-group-main">
      <div class="filelist-group-main-header" v-if="header">
        ...
        <div class="folder-operations" v-if="bindFolderEditable">
          <page-button @click="onUpdateFolderClick">修改目录</page-button>
          <page-button theme="default" @click="onDeleteFolderClick" :disabled="!!bindFiles.length">删除目录</page-button>
        </div>
      </div>
              ...
              <div>
                ...
                <div class="file-info-content">
                  <div class="file-info-content-row">
                    ...
                    <svg-icon v-if="!readOnly" icon-class="edit2" class="edit-filename" @click="onEditNameClick(item, index)"></svg-icon>
                  </div>
    ...
    <vxe-modal v-model="form.visible"
      class="popup-form-modal popup-form-modal--scroll"
      title="新建目录"
      :width="518"
      :position="{ top: '12vh' }"
      type="confirm"
      ...
      @hide="form.data = {};"
      @confirm="form.ok"
      @show="onFolderModalShow"
    >
      <div class="modal-folder-inner">
        <svg-icon icon-class="folder" fill="#FFC121" size="80"></svg-icon>
        <span>目录名称</span>
        <vxe-form style="width: 100%; margin-top: 10px;"
          :data="form.data"
          :rules="form.rules"
          :span="24"
          @submit="form.ok"
          @reset="form.visible = false;"
          ref="folderForm"
        >
          <vxe-form-gather :span="24">
            <vxe-form-item field="name" class-name="folder-name-input">
              <template #default="{ data }">
                <vxe-input v-model="data.name" placeholder="请输入名称" clearable ref="folderInput"></vxe-input>
              </template>
            </vxe-form-item>
          </vxe-form-gather>
          <vxe-form-item align="right" span="24" class-name="modal-folder-footer">
            <template #default>
    ...
    <popup-form v-model="folderForm.visible"
      title="修改目录"
      :span="12"
      width="450"
      :form-data="folderForm.data"
      :data-gather="folderForm.dataGather"
      :rules="folderForm.rules"
      :title-width="90"
      auto-scroll
      okText="确定"
      cancelText="取消"
...
export default {
  ...
  data() {
    return {
      ...
      operationList: [
        { name: '移动', code: 'move', icon: 'moveto', excute: true },
        { name: '在线预览', code: 'preview', icon: 'preview' },
        { name: '下载', code: 'download', icon: 'download2' },
        { name: '删除', code: 'delete', icon: 'garbage', excute: true },
      ],
