...
export default {
  ...
  methods: {
    async getData(callback) {
      // this.fileList = this.testData.fileList ?? [];
      // this.groups = (this.testData?.folderList ?? []).map((item) => {
      //   return {
      //     id: item.id,
      ...
      //   };
      // });
      const { billId, billType ,menuCode} = this
      const res = await ApiBillFile.getBillAuthFiles(billId, billType,menuCode)
    ...
    async onExportBtnClick() {
      ...
      const res = await ApiBillFile.exportAuthFiles(
        billId,
        billType,
        currentFolderId,
        menuCode
    ...
    onEventUpload({ files, groupId, next }) {
      ...
      const data = {
        ...
        folderCode:this.groups.find(x => x.id == groupId)?.folderCode,
    ...
    //   const { billId, billType ,menuCode} = this;
    //   ApiBillFile.addFolder(billId, billType, name,menuCode).then((res) => {
    ...
    //   const { billId, billType,menuCode } = this;
    //   ApiBillFile.deleteFolder(id, billId, billType,menuCode).then((res) => {
    //     if (res.code === 200) {
    //       this.$message.success(res.message);
    //       next();
    //     } else {
    ...
    onEventUpdateFilename({ item, name, next }) {
      const { billId, billType,menuCode } = this;
      ApiBillFile.updateBillAuthFlieName({
        fileId: item.id,
        ...
        folderCode:item.folderAuth.folderCode,
      }).then((res) => {
        if (res.code === 200) {
          this.$message.success(res.message);
          next();
