...
const service = axios.create({
  // 外网更新
  // baseURL: 'http://xxxxx',
  baseURL: 'http://192.168.3.121:9004',  
...
/* 2.全局默认配置 */
let baseURL// = 'https://lxqapcloud.tpddns.cn:18096/api'
// 判断开发环境（一般用于本地代理）
if (process.env.NODE_ENV === 'development') { // 开发环境
    baseURL = '/apis'    // 你设置的本地代理请求（跨域代理），下文会详细介绍怎么进行跨域代理
} else {                                      // 编译环境
...
Axios.interceptors.response.use((res) =>{
   //请求响应后拦截
   console.log("11",Axios.baseURL)
   if(res.status == 200 || res.status == 405){                       // 对响应数据做些事
       //alert("提交成功")
